...
import 'package:fitness_tracker/meal_plan.dart';
...
class _ScreensState extends State<Screens> {
  ...
  Widget build(context) {
    ...
    if (levels[0]
        .finished
        .every((value) => value == 2 && check[0] == 0 && completed[0] != 5)) {
      completed[0]++;
      check[0]++;
    }
    if (levels[1]
        .finished
        .every((value) => value == 2 && check[1] == 0 && completed[1] != 5)) {
      completed[1]++;
      check[1]++;
    }
    if (levels[2]
        .finished
        .every((value) => value == 2 && check[2] == 0 && completed[2] != 5)) {
      completed[2]++;
      check[2]++;
    }
    ...
    if (levels[currentLevel]
        .finished
        .every((value) => value == 2 && completed[currentLevel] != 5)) {
      waiting();
    }
    ...
    if (screenSwitch == 'workouts-screen') {
      activeScreen =
          WorkoutsScreen(noOfLevel, completed, resetLevels, goBackToGoals);
    }
    if (screenSwitch == 'goal-screen') {
      activeScreen = GoalsScreen(
          noOfLevel[currentLevel],
          nameOfLevel,
          nameOfNextLevel,
    ...
    if (screenSwitch == 'exercise-screen') {
      activeScreen = ExerciseScreen(
          noOfLevel[currentLevel], index, goBackToGoals, increaseFinished);
    }
    if (screenSwitch == 'instruction-screen') {
      activeScreen = const InstructionScreen();
    }
    if (screenSwitch == 'meal_plan-screen') {
      activeScreen = const MealPlan();
    }
